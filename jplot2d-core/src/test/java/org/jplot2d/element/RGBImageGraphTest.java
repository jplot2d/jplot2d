 ...
 * @author Jingjing Li
 */
public class RGBImageGraphTest {
    ...
    public void testCreateImageGraph() {
        RGBImageGraph graph = factory.createRGBImageGraph(null);

        assertNull(graph.getSize());
        assertNull(graph.getBounds());
        assertNull(graph.getSelectableBounds());

        RGBImageMapping mapping = graph.getMapping();
        ...
        assertArrayEquals(mapping.getGraphs(), new RGBImageGraph[]{graph});
    ...
    public void testSetMapping() {
        ...
        RGBImageGraph graph0 = factory.createRGBImageGraph(null);
        assertNotNull(graph0.getMapping());
        assertArrayEquals(graph0.getMapping().getGraphs(), new RGBImageGraph[]{graph0});

        // set mapping before added to a layer
        RGBImageMapping mapping1 = factory.createRGBImageMapping();
        ...
        RGBImageMapping mapping2 = factory.createRGBImageMapping();
    ...
    public void testShareMapping() {
        ...
        RGBImageGraph graph0 = factory.createRGBImageGraph(null);
        RGBImageGraph graph1 = factory.createRGBImageGraph(null);
        assertNotNull(graph0.getMapping());
        ...
        assertArrayEquals(graph0.getMapping().getGraphs(), new RGBImageGraph[]{graph0, graph1});
        assertSame(graph0.getMapping().getEnvironment(), p.getEnvironment());
        ...
        // set mapping
        graph1.setMapping(mapping1);
        assertArrayEquals(graph1.getMapping().getGraphs(), new RGBImageGraph[]{graph1});
