...
public class GeometryUtil {
	...
	public static int intersectLinesWithParams(double x1, double y1, double x2, double y2,
			double x3, double y3, double x4, double y4, double[] params) {
		double dx = x4 - x3;
		double dy = y4 - y3;
	...
	/**
	 * The method checks up if line (x1, y1) - (x2, y2) and line (x3, y3) - (x4, y4) intersect. If
	 * lines intersect then the result parameters are saved to point array. The size of array point
	 * must be at least 2.
	 * 
	 * @return the method returns 1 if two lines intersect in the defined interval, otherwise 0
	 */
	public static int intersectLines(double x1, double y1, double x2, double y2, double x3,
			double y3, double x4, double y4, double[] point) {
		double A1 = -(y2 - y1);
		double B1 = (x2 - x1);
	...
	/**
	 * It checks up if there is intersection of the line (x1, y1) - (x2, y2) and the quad curve
	 * (qx1, qy1) - (qx2, qy2) - (qx3, qy3). The parameters of the intersection area saved to params
	 * array. Therefore the params size must be at learst 4.
	 * 
	 * @return The method returns the quantity of roots lied in the defined interval
	 */
	public static int intersectLineAndQuad(double x1, double y1, double x2, double y2, double qx1,
	...
	/**
	 * It checks up if the line (x1, y1) - (x2, y2) and the cubic curve (cx1, cy1) - (cx2, cy2) -
	 * (cx3, cy3) - (cx4, cy4). The points of the intersection is saved to points array. Therefore
	 * the points size must be at learst 6.
	 * 
	 * @return The method returns the quantity of roots lied in the defined interval
	 */
	public static int intersectLineAndCubic(double x1, double y1, double x2, double y2, double cx1,
			double cy1, double cx2, double cy2, double cx3, double cy3, double cx4, double cy4,
			double[] params) {
		...
		eqn[3] = (-3 * cy1 + 3 * cy2 - 3 * cy3 + cy4) * dx + (3 * cx1 - 3 * cx2 + 3 * cx3 - cx4)
				* dy;
	...
	/**
	 * The method checks up if two quads (x1, y1) - (x2, y2) - (x3, y3) and (qx1, qy1) - (qx2, qy2)
	 * - (qx3, qy3) intersect. The result is saved to point array. Size of points should be at
	 * learst 4.
	 * 
	 * @return the method returns the quantity of roots lied in the interval
	 */
	public static int intersectQuads(double x1, double y1, double x2, double y2, double x3,
			double y3, double qx1, double qy1, double qx2, double qy2, double qx3, double qy3,
			double[] params) {
	...
	/**
	 * It checks up if the quad (x1, y1) - (x2, y2) - (x3, y3) and the cubic (cx1, cy1) - (cx2, cy2)
	 * - (cx3, cy3) - (cx4, cy4) curves intersect. The points of the intersection is saved to points
	 * array. The points size should be at learst 6.
	 * 
	...
	public static int intersectQuadAndCubic(double qx1, double qy1, double qx2, double qy2,
			double qx3, double qy3, double cx1, double cy1, double cx2, double cy2, double cx3,
			double cy3, double cx4, double cy4, double[] params) {
		int quantity = 0;
	...
	/**
	 * The method checks up if two cubic curves (x1, y1) - (x2, y2) - (x3, y3) - (x4, y4) and (cx1,
	 * cy1) - (cx2, cy2) - (cx3, cy3) - (cx4, cy4) intersect. The result is saved to point array.
	 * Size of points should be at learst 6.
	 * 
	 * @return the method returns the quantity of the intersection points lied in the interval
	 */
	public static int intersectCubics(double x1, double y1, double x2, double y2, double x3,
			double y3, double x4, double y4, double cx1, double cy1, double cx2, double cy2,
			double cx3, double cy3, double cx4, double cy4, double[] params) {
	...
	public static double cubic(double t, double x1, double x2, double x3, double x4) {
		return x1 * (1.0 - t) * (1.0 - t) * (1.0 - t) + 3.0 * x2 * (1.0 - t) * (1.0 - t) * t + 3.0
				* x3 * (1.0 - t) * t * t + x4 * t * t * t;
	}
	...
	private static void cubicNewton(double xCoefs1[], double yCoefs1[], double xCoefs2[],
		...
		while (Math.sqrt((t - t1) * (t - t1) + (s - s1) * (s - s1)) > EPSILON) {
			...
			dt = (t * t * t * xCoefs1[0] + t * t * xCoefs1[1] + t * xCoefs1[2] + xCoefs1[3] - s * s
					* s * xCoefs2[0] - s * s * xCoefs2[1] - s * xCoefs2[2] - xCoefs2[3])
					* (-3 * s * s * yCoefs2[0] - 2 * s * yCoefs2[1] - yCoefs2[2])
	...
	private static void quadAndCubicNewton(double xCoefs1[], double yCoefs1[], double xCoefs2[],
			double yCoefs2[], double[] params) {
		double t = 0.0, s = 0.0;
		double t1 = params[0];
		double s1 = params[1];
		...
		while (Math.sqrt((t - t1) * (t - t1) + (s - s1) * (s - s1)) > EPSILON) {
			...
			dt = (t * t * xCoefs1[0] + t * xCoefs1[1] + xCoefs1[2] + -s * s * s * xCoefs2[0] - s
	...
	private static void quadNewton(double xCoefs1[], double yCoefs1[], double xCoefs2[],
		...
		while (Math.sqrt((t - t1) * (t - t1) + (s - s1) * (s - s1)) > EPSILON) {
			...
			dt = -(t * t * xCoefs1[0] + t * xCoefs1[1] + xCoefs1[1] - s * s * xCoefs2[0] - s
					* xCoefs2[1] - xCoefs2[2])
					* (2 * s * yCoefs2[0] + yCoefs2[1])
					+ (2 * s * xCoefs2[0] + xCoefs2[1])
					* (t * t * yCoefs1[0] + t * yCoefs1[1] + yCoefs1[2] - s * s * yCoefs2[0] - s
							* yCoefs2[1] - yCoefs2[2]);
			...
			ds = (2 * t * xCoefs1[0] + xCoefs1[1])
					* (t * t * yCoefs1[0] + t * yCoefs1[1] + yCoefs1[2] - s * s * yCoefs2[0] - s
							* yCoefs2[1] - yCoefs2[2])
					- (2 * t * yCoefs1[0] + yCoefs1[1])
					* (t * t * xCoefs1[0] + t * xCoefs1[1] + xCoefs1[2] - s * s * xCoefs2[0] - s
							* xCoefs2[1] - xCoefs2[2]);
