...
public class LogarithmicTransformType implements TransformType {
	...
	public AxisTransform createTransform(Range2D pr, Range2D ur) {
		AxisTransform result = new LogarithmicAxisTransform(pr, ur);
		if (!result.isValid()) {
	...
	public boolean isTransformType(AxisTransform trf) {
		return trf instanceof LogarithmicAxisTransform;
	}
	...
	public boolean isTransformType(NormalTransform nptrf) {
		return nptrf instanceof LogarithmicNormalTransform;
	}
