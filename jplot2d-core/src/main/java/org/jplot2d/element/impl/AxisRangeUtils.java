...
public class AxisRangeUtils {
    ...
    /**
     * The depth of zoom-in is controlled by this constant.
     * The precision limit of double is 0x1.0p-52. 65536x to ensure the ticks look like even-distributed.
     */
    protected static final double PRECISIONLIMIT = 0x1.0p-36;

    /**
     * The default precision limit when range == 0.
     * This make the range is large enough to produce tick labels within 4 digits.
     */
     ...
     *
     * @param vtMap a map from axes to their normalized 0-offset transform
     * @return a range that is valid in all axes
     */
    ...
    /**
     * Create a set of virtual NormalTransform correspond to the NormalTransform of the given axes.
     * The virtual NormalTransform will try to remove the offset to minimize the double calculation round error.
     *
     * @param axes the axes
     * @return a map from axes to their normalized 0-offset transform
     */
    public static Map<AxisTransformEx, NormalTransform> createVirtualTransformMap(Collection<AxisTransformEx> axes) {
        ...
        Map<AxisTransformEx, NormalTransform> result = new HashMap<>();
    ...
    public static Map<AxisTransformEx, NormalTransform> createNormalTransformMap(Collection<AxisTransformEx> axes) {
        Map<AxisTransformEx, NormalTransform> vtMap = new LinkedHashMap<>();
        for (AxisTransformEx axis : axes) {
     ...
     * </ul>
     *
     * @return a RangeStatus contain the nice normalized range. The Boolean status to indicate if there are some data points outside the value bounds.
     */
    public static RangeStatus<Boolean> calcNiceVirtualRange(Map<AxisTransformEx, NormalTransform> vtMap) {
        ...
        return new RangeStatus<>(padRange.getMin(), padRange.getMax(), dataOutsideBounds);
     ...
     * <li>If all corresponding world range of layers are valid, the given norm range is returned without any adjustment.</li>
     * <li>If any corresponding world range of layers spans over invalid value, the end of given range is adjusted inward to the valid boundary.</li>
    ...
    /**
     * Find a nice norm range inside the given range and to make sure all corresponding user range only contain valid values.
     * <ul>
     * <li>If there is no intersection among the given range and valid boundaries, null is returned.</li>
     * <li>If all corresponding world range of layers are valid, the given norm range is returned without any adjustment.</li>
     * <li>If any corresponding world range of layers spans over invalid value, the end of given range is adjusted inward to the data that most close to boundary.
     * <ul>
     * <li>If there is no valid data inside the valid range, <code>null</code> will be returned.</li>
     * <li>A margin is appended on the "nearest data" end.</li>
     * </ul>
    ...
    /**
     * If there is no data point in the given range, null is returned.
     * The range on the end of found data point will expand by a margin factor.
     *
     * @param pbnds      The caller guarantee this is valid on all axes.
     * @param axisMap    a map from axes to their normalized 0-offset transform
     * @param findLoData indicate the low data should be found
     * @param findHiData indicate the high data should be found
     * @return a new range
     */
     ...
     *
     * @param prange the normalized range
     * @param axes   the axes
     * @return the new range that satisfy the precision limit. The status object is PrecisionException if the range is adjusted.
     */
    ...
    public static RangeStatus<PrecisionState> ensurePrecision(Range prange, Map<AxisTransformEx, NormalTransform> vtMap) {
        ...
        return new RangeStatus<>(pLo, pHi, ex);
    }
