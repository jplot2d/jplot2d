 ...
 * string or composite of other components. The laying out operate at device
...
public abstract class MathLabelComp {
    ...
    private static class Ptoken extends MathLabelComp {
        ...
        private double getStrLogicalWidth() {
            ...
            double dwidth = _font.getStringBounds(_str, frc).getWidth();
            return dwidth;
        }
    ...
    private static class Msup extends MathLabelComp {
        MathLabelComp _base, _sup;

        private Msup(MathElement.Msup me, double x, double y, Font font,
                boolean wstyle) {
            super(me, font, x, y);
            _base = MathLabelComp
                    .getBoxInstance(me.base, _x, _y, _font, wstyle);
            ...
            _sup = MathLabelComp.getBoxInstance(
                    ((MathElement.Msup) _me).superscript, suplocx, suplocy,
                    supfont, wstyle);
    ...
    private static class PContainer extends MathLabelComp {
        ...
        private PContainer(MathElement.PContainer me, double x, double y,
            ...
            MathElement.PElement[] es = me.getElements();
            double nextLocx = _x;
            for (int i = 0; i < es.length; i++) {
                MathLabelComp ml = MathLabelComp.getBoxInstance(es[i],
                        nextLocx, _y, _font, wstyle);
                _subEL.add(ml);
                nextLocx += ml._width;
    ...
    /**
     * Static factory method. Create a proper subclass according to the given
     * MathElement argument.
     * 
     * @param me
     * @param font
     *            the screen font
     ...
     * @param y
     */
    protected static MathLabelComp getBoxInstance(MathElement me, double x,
            double y, Font font, boolean wstyle) {
        if (me instanceof MathElement.PToken) {
            ...
            if (me instanceof MathElement.Mstyle) {
                wstyle = true;
                font = deriveMathVariantFont(font, ((MathElement.Mstyle) me)
                        .getMathVariant());
            }
            return new PContainer(((MathElement.PContainer) me), x, y, font,
                    wstyle);
        } else {
            return null;
        }
    ...
    private static Font deriveMathVariantFont(Font font,
            MathElement.MathVariant mathVariant) {
        switch (mathVariant) {
        case NORMAL:
     ...
     * 
     * @param me
     *            the MathElement object. can be null if the parsed text is
     *            empty string.
     * @param font
     *            the screen font
     * @param x
