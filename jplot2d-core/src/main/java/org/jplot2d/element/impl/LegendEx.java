...
public interface LegendEx extends Legend, ComponentEx {
    ...
    /**
     * Add the given item to this legend. If this legend is disabled, the item should be added to its upper level legend.
     * This method will set the legend property of added legend item to the actual hosting legend.
     *
    ...
    /**
     * Called by a legend item to notify this legend that the item's visibility is changed.
     * An item is shown when both visibility and contributivity are <code>true</code>.
     */
    void itemVisibilityChanged(LegendItemImpl item);

    /**
     * Called by a legend item to notify that the item's contributivity is changed.
     * An item is shown when both visibility and contributivity are <code>true</code>.
     */
    void itemContribitivityChanged(LegendItemImpl item);

    /**
     * Returns the thickness of this legend. When position is on top or bottom, thickness is its height.
     * When position is on left or right, thickness is its width.
     *
